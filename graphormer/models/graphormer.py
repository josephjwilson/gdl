...
class GraphormerEncoder(FairseqEncoder):
    def __init__(self, args):
        ...
        self.graph_encoder = GraphormerGraphEncoder(
            ...
            fixed_spd_bias=getattr(args, 'fixed_spd_bias', False),
            alibi_spd_bias=getattr(args, 'alibi_spd_bias', False),
            alibi_custom_slopes=getattr(args, 'alibi_custom_slopes', None),
            use_ffn=getattr(args, 'use_ffn', True),
            causal_mask=getattr(args, 'causal_mask', False),
            use_virtual_distance=getattr(args, 'use_virtual_distance', True),
            no_cls=getattr(args, 'no_cls', False),
            no_degree_embedding=getattr(args, 'no_degree_embedding', False),
            use_ecc_cls_bias=getattr(args, 'use_ecc_cls_bias', False),
            ecc_cls_slope=getattr(args, 'ecc_cls_slope', 1.0),
            use_farness_cls_bias=getattr(args, 'use_farness_cls_bias', False),
            use_ecc_alibi_cls_bias=getattr(args, 'use_ecc_alibi_cls_bias', False),
            use_raw_ecc_alibi_cls_bias=getattr(args, 'use_raw_ecc_alibi_cls_bias', False),
            raw_ecc_alibi_slope_rate=getattr(args, 'raw_ecc_alibi_slope_rate', 8.0),
            alibi_spd_slope_rate=getattr(args, 'alibi_spd_slope_rate', 8.0),
            use_random_cls_bias=getattr(args, 'use_random_cls_bias', False),
        )
    ...
    def forward(self, batched_data, perturb=None, masked_tokens=None, **kwargs):
        inner_states, graph_rep, attn_weights = self.graph_encoder(
            batched_data,
            perturb=perturb,
            **kwargs,
        )
