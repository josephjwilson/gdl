...
class GraphormerGraphEncoder(nn.Module):
    ...
    def forward(
        ...
        # B x (T+1) with CLS, or B x T without
        ...
        # Generate causal mask if enabled (upper-triangular -inf)
        if self.causal_mask and attn_mask is None:
            T = x.size(0)
        ...
        if self.no_cls:
            # Mean-pool over non-padded nodes: x is [T, B, C]
            valid = (~padding_mask).float().t().unsqueeze(-1)  # [T, B, 1]
            graph_rep = (x * valid).sum(dim=0) / valid.sum(dim=0).clamp(min=1)  # [B, C]
        else:
            graph_rep = x[0, :, :]
        ...
        res_attn = attn_weights if return_attn else None

        if self.traceable:
            return torch.stack(inner_states), graph_rep, res_attn
        else:
            return inner_states, graph_rep, res_attn
